<?php
    $iddosen = $_GET["p1"];
    $sql = "SELECT tm.nama, tm.nidn, tm.notelpon, tm.alamat, tm.iddosen FROM tbdosen tm WHERE tm.iddosen='$iddosen';";
    $hasil = mysqli_query($cnn, $sql);
    if(mysqli_num_rows($hasil) > 0){
        $h = mysqli_fetch_assoc($hasil);
?>
<h3>Ubah Data Dosen <?=$h["nidn"]?></h3>
<form method="POST" action="datadosen.php">
    ...
    <div class="form-floating mb-3">
        <input type="text" name="txNIDN" class="form-control" id="floatingInput" placeholder= "NIDN " value="<?=$h["nidn"]?>">
        <label for="floatingInput">NIDN</label>
    </div>
    <div class="form-floating mb-3">
        <input type="text" name="txNOTELPON" class="form-control" id="floatingInput" placeholder="No Telpon" value="<?=$h["notelpon"]?>">
